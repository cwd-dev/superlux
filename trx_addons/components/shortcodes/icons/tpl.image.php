	...
	foreach ($args['icons3'] as $item) {
		...
		?><div class="sc_icons_item<?php echo !empty($item['link']) ? ' sc_icons_item_linked' : ''; ?>"><?php
			if (!empty($item['image3'])) {
				$item['image3'] = trx_addons_get_attachment_url($item['image3'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('classic'), 'icons-image'));
				if (!empty($item['image3'])) {
					$attr = trx_addons_getimagesize($item['image3']);
					?><div class="sc_icons_image"><img src="<?php echo esc_url($item['image3']); ?>" alt="<?php echo esc_attr(basename($item['image3'])); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
			...
			?><?php
			if ((!empty($item['title3'])) || (!empty($item['description3']))) {
			?><div class="sc_icons_item_content">
			<?php
			}
			?>
			<?php
			if (!empty($item['title3'])) {
				$item['title3'] = explode('|', $item['title3']);
				?><h4 class="sc_icons_item_title"><?php
					foreach ($item['title3'] as $str) {
						?><span><?php trx_addons_show_layout($str); ?></span><?php
					}
				?></h4><?php
			}
			if (!empty($item['description3'])) {
				?><div class="sc_icons_item_description"><?php
					if (strpos($item['description3'], '<p>') === false) {
						$item['description3'] = explode('|', str_replace("\n", '|', $item['description3']));
						foreach ($item['description3'] as $str) {
							?><span><?php trx_addons_show_layout($str); ?></span><?php
						}
					} else
						trx_addons_show_layout($item['description3']);
				?></div><?php
			}
		?>
		<?php
