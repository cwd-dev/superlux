...
if ( has_post_thumbnail() ) {
	if (isset($args['service_compact']) && ($args['service_compact'] > 0)){
		$image = trx_addons_get_attachment_url( get_post_thumbnail_id( get_the_ID() ), trx_addons_get_thumb_size('servicecompact') );
	} else {
		$image = trx_addons_get_attachment_url( get_post_thumbnail_id( get_the_ID() ), trx_addons_get_thumb_size('serviceimage') );
	}
}
...
<div class="sc_services_item<?php
	...
	if (isset($args['service_compact']) && ($args['service_compact'] > 0)) echo ' sc_services_compact ';
?> <?php if (!empty($image)) echo esc_attr(soleng_add_inline_css_class('background-image:url('.esc_url($image).');')); ?>"<?php
	if (!empty($args['popup'])) {
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
																...
																'hover' => 'zoomin',
																'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'servicecompact' : 'servicecompact'), 'services-image'),
																'post_info' => '<span class="sc_services_description"><span class="sc_services_description_content">'. get_the_excerpt() .'</span></span>'
																),
															'services-image'
															)
										);
				...
				trx_addons_get_template_part('templates/tpl.featured.php',
												...
												apply_filters('trx_addons_filter_args_featured', array(
																	'class' => 'sc_services_item_thumb',
																	'hover' => 'zoomin',
																	'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'serviceimage' : 'serviceimage'), 'services-image'),
																	'post_info' => '<span class="sc_services_description"><span class="sc_services_description_content">'. get_the_excerpt() .'</span></span>'
																	),
																'services-image'
																)
			...
			?>><?php
				if (!empty($svg)) {
					...
					?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php echo esc_attr(basename($img)); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			?></a><?php
		} else {
