...
if (!function_exists('soleng_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'soleng_hovers_theme_setup3', 3 );
	function soleng_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		soleng_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'soleng'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'soleng') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'soleng'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'soleng'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'soleng') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
...
if (!function_exists('soleng_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'soleng_hovers_theme_setup9', 9 );
	function soleng_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'soleng_hovers_frontend_scripts', 1010 );
		add_filter( 'soleng_filter_localize_script','soleng_hovers_localize_script' );
		add_filter( 'soleng_filter_merge_scripts',	'soleng_hovers_merge_scripts' );
		add_filter( 'soleng_filter_merge_styles',	'soleng_hovers_merge_styles' );
		add_filter( 'soleng_filter_merge_styles_responsive', 'soleng_hovers_merge_styles_responsive' );
		add_filter( 'soleng_filter_get_css', 		'soleng_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'soleng_hovers_frontend_scripts' ) ) {
	
	function soleng_hovers_frontend_scripts() {
		if ( soleng_is_on(soleng_get_theme_option('debug_mode')) && soleng_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'soleng-hovers', soleng_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('soleng_hovers_merge_styles')) {
	
	function soleng_hovers_merge_styles($list) {
...
if (!function_exists('soleng_hovers_merge_styles_responsive')) {
	
	function soleng_hovers_merge_styles_responsive($list) {
...
if (!function_exists('soleng_hovers_localize_script')) {
	
	function soleng_hovers_localize_script($arr) {
		$arr['button_hover'] = soleng_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('soleng_hovers_merge_scripts')) {
	
	function soleng_hovers_merge_scripts($list) {
...
if ( !function_exists('soleng_hovers_add_icons') ) {
	function soleng_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					soleng_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'soleng') : esc_html__('Buy now', 'soleng')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'soleng');
						else
							esc_html_e('Details', 'soleng');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-image_hover"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'soleng_hovers_get_css' ) ) {
	
	function soleng_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.post_featured.hover_icon .mask {
	background-color: {$colors['text_link2_09']};
}
